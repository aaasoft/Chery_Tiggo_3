...
void PlayButtonSound()
{
	PlaySound(MAKEINTRESOURCE(IDR_WAVE_BTN),NULL,SND_RESOURCE|SND_ASYNC|SND_NODEFAULT);
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDB_EASYCONNECTED:
			...
			if(g_MainScreenHiden)
			{
				...
				if(hTemp)
					ShowWindow(hTemp,SW_HIDE);
				SetWindowText(hBtnCarLife,STRING_BTN_EASYCONNECTED_TITLE);
			}
			else
			{
				//显示CarLife
				hTemp = FindWindow(NULL,STRING_EASYCONNECTED_WINDOW_TITLE);
				if(!hTemp)
					CreateProcess(STRING_EASYCONNECTED_FULL_FILE_NAME,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
				else				
					ShowWindow(hTemp,SW_SHOW);
				//隐藏主界面
				ShowCheryWindow(false);
	...
	case WM_CREATE:
		...
		hBtnCarLife = CreateWindow(L"Button", STRING_BTN_EASYCONNECTED_TITLE, WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON,
			g_ButtonTotalWidth, 0, g_ButtonWidth, g_ButtonWidth,
			hWnd, (HMENU)IDB_EASYCONNECTED, (HINSTANCE)hWnd, NULL);
